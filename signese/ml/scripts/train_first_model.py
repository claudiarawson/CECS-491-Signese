...
from ml.training.dataset import DEFAULT_SPLITS, LandmarkSequenceDataset, create_dataloader
...
def build_datasets(split_paths: dict[str, Path], label_mapping_path: Path) -> tuple[dict[str, LandmarkSequenceDataset], list[str]]:
    ...
    datasets = {
        split: LandmarkSequenceDataset(path, label_mapping_path=label_mapping_path)
...
def main() -> None:
    ...
    dataloaders = {
        "train": create_dataloader(datasets["train"], batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS),
