"""
export_asl_citizen_landmark_extraction_jobs.py
----------------------------------------------
Prepare landmark extraction job files for the ASL Citizen reduced 8-label
Signese experiment.
...
Outputs:
  ...
  - data/processed/landmarks/asl_citizen_job_summary.json
...
SUMMARY_OUT = LANDMARKS_DIR / "asl_citizen_job_summary.json"
...
def make_output_path(split: str, label: str, sample_id: str) -> str:
    ...
    # Keep ASL Citizen features isolated from WLASL feature outputs.
    out = FEATURES_DIR / "asl_citizen" / split / label_dir / f"{safe_sample_id}.npy"
