"""
run_custom_landmark_extraction.py
--------------------------------
Run landmark extraction for custom reduced-label job manifests using MediaPipe
Holistic with the same feature format as existing pipelines.

Inputs:
  - data/processed/landmarks/custom_train_jobs.json
  - data/processed/landmarks/custom_val_jobs.json
  - data/processed/landmarks/custom_test_jobs.json

Outputs:
  - data/processed/landmarks/features/custom/<split>/<LABEL>/<sample_id>.npy
  - data/processed/landmarks/custom_extraction_report.json
"""
...
INPUTS = {
    "train": LANDMARKS_DIR / "custom_train_jobs.json",
    "val": LANDMARKS_DIR / "custom_val_jobs.json",
    "test": LANDMARKS_DIR / "custom_test_jobs.json",
}
REPORT_OUT = LANDMARKS_DIR / "custom_extraction_report.json"
...
def build_holistic_for_job(example_job: dict[str, Any]) -> Any:
    ...
    # TODO(unify-runners): Merge WLASL, ASL Citizen, and custom runners into one configurable runner.
    # TODO(parallelism): Add multi-worker extraction for faster custom refresh cycles.
    # TODO(extractor-backend): Support backend swapping (e.g. MediaPipe Tasks/OpenPose) via config.
...
def main() -> None:
    ...
    print(f"[ok] Extraction report -> {REPORT_OUT.relative_to(REPO_ROOT)}")
    print()
