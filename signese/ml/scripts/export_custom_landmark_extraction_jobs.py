"""
export_custom_landmark_extraction_jobs.py
-----------------------------------------
Prepare landmark extraction jobs for targeted custom reduced-label samples.
...
Outputs:
  ...
  - data/processed/landmarks/custom_job_summary.json
"""
...
SUMMARY_OUT = LANDMARKS_DIR / "custom_job_summary.json"
...
def make_output_path(split: str, label: str, sample_id: str) -> str:
    ...
    out = FEATURES_DIR / "custom" / split / label_dir / f"{safe_sample_id}.npy"
        ...
        jobs.append(
            {
                ...
                "source_gloss": row.get("source_gloss", label),
                "video_path": str(video_path),
...
def main() -> None:
    ...
    print(f"[ok] Custom train jobs -> {OUTPUTS['train'].relative_to(REPO_ROOT)}")
    print(f"[ok] Custom val jobs   -> {OUTPUTS['val'].relative_to(REPO_ROOT)}")
    print(f"[ok] Custom test jobs  -> {OUTPUTS['test'].relative_to(REPO_ROOT)}")
    print(f"[ok] Custom summary    -> {SUMMARY_OUT.relative_to(REPO_ROOT)}")
    print()
